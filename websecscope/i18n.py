...
FINDING_TRANSLATIONS = {
    "ko": {
        "WEB_HEADER_CONTENT_SECURITY_POLICY": {
            ...
            "description": (
                "CSP는 브라우저에서 허용되는 스크립트, 스타일, 리소스 출처를 제한해 "
                "XSS 영향을 줄입니다."
            ),
            "recommendation": (
                "웹 서버, 리버스 프록시 또는 애플리케이션에서 서비스에 맞는 "
        ...
        "WEB_HEADER_X_FRAME_OPTIONS": {
            ...
            "description": (
                "X-Frame-Options는 클릭재킹 방지를 위해 페이지가 frame/iframe에 "
                "로드되는 방식을 제한합니다."
            ),
            "recommendation": (
                "DENY 또는 SAMEORIGIN 정책을 설정하거나 CSP frame-ancestors를 "
                "함께 검토하세요."
            ),
        },
        "WEB_SENSITIVE_PATHS": {
            "title": "민감 경로 노출 점검",
            "description": (
                "저장소, 설정 파일, 백업, 관리자 후보 경로의 HTTP 응답 상태를 "
                "근거와 해석으로 분리해 평가합니다."
            ),
            "recommendation": (
                "관리자, 저장소, 백업, 설정 경로는 공개 접근을 차단하고 필요한 경우 "
                "인증 및 네트워크 제한을 적용하세요."
            ),
        },
        "AUTH_MAY_BE_MISSING": {
        ...
        "CVE_SERVICE_INVENTORY": {
            ...
            "description": (
                "CVE 조회는 탐지된 제품/버전 근거를 기반으로 하며 수동 검증 전에는 "
                "참고 정보입니다."
            ),
            "recommendation": (
                "서비스 배너 또는 패키지 인벤토리를 수집해 CVE 매칭 정확도를 높이세요."
            ),
        },
    },
    "en": {},
...
RECOMMENDATION_TRANSLATIONS = {
    "ko": {
        ...
        "Review the finding and apply the least-privilege secure configuration recommended by the service vendor.": (
            "벤더 권장 보안 설정과 최소 권한 원칙에 맞춰 해당 항목을 검토하세요."
        ),
        "Provide a fully qualified authorized URL.": (
            "권한이 있는 완전한 URL(http:// 또는 https:// 포함)을 입력하세요."
        ),
